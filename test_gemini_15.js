...
async function analyzeWithGemini(base64Image, apiKey, model) {
...
async function run() {
  ...
  try {
    const res = await analyzeWithGemini(base64Image, GEMINI_API_KEY, 'gemini-2.5-flash');
    console.log("DMFT:", res.dmft_score);
    ...
    console.log("Non-healthy teeth:", JSON.stringify(nonHealthy, null, 2));
  } catch (err) {
