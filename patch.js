...
window.renderSavedNotesDashboard = function() {
  ...
  if (user && window.DB) {
    ...
    scans.forEach(s => {
      if (s.researchNotes) {
        s.researchNotes.forEach(n => {
          allNotes.push({ id: n.id, source: 'Case Study', context: 'Scan: ' + s.patientId, content: n.content, date: n.date });
        });
      }
    });
  }
...
console.log('Appended to researcherModule.js');
